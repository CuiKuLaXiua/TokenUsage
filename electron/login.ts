...
export class LoginWindowManager {
  ...
  private loginCompleteCallback: ((cookies: string | null) => void) | null = null
  ...
  async openLoginWindow(url: string, parentWindow?: BrowserWindow): Promise<void> {
    ...
    console.log('[LoginWindow] 清除后剩余 cookies:', remainingCookies.length, '个')
    ...
    this.loginWindow.webContents.on('did-finish-load', () => {
      console.log('[LoginWindow] 页面加载完成，3秒后检查 cookies')
      setTimeout(() => {
    ...
    try {
      ...
      if (cookies.length > 0) {
        ...
        for (const cookie of cookies) {
          console.log(`[LoginWindow] Cookie: ${cookie.name}=${cookie.value} (domain: ${cookie.domain}, path: ${cookie.path})`)
        }
  ...
  private async extractCookiesFromSession(): Promise<void> {
    ...
    try {
      ...
      console.log('[LoginWindow] 从 session 提取 cookies:', cookies.length, '个')
      if (cookies.length > 0) {
        // 打印所有 cookie 完整信息
        for (const cookie of cookies) {
          console.log(`[LoginWindow] Cookie: ${cookie.name}=${cookie.value} (domain: ${cookie.domain}, path: ${cookie.path})`)
